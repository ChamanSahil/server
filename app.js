...
try {
    descopeClient = DescopeClient({ projectId: 'P2cch9UzY4dawVO5pnR3dTLI8SXG', managementKey: K2cg3GXGAnSPzrFnfQ5lgg9F12QrboLhtdokzh5Z5lkv1gGJDho5KC3oxV6C2NA6lP5SFWG });
} catch (error) {
...
async function nudgeUsers(gender, ageGroup, name, link, desc, price, company, stream, image) {
  const users = await db('e_users').select('*')
...
app.post("/webhook", async (req, res) => {
  ...
  if (req.body.object) {
    if (
      ...
      try {
        const users = await db('e_users').select('*').where({uPhoneNo: from})
        if(!users.length) {
          ...
          // list companies
          if(msg === "brands") {
            const brands = await db('e_brands').select('bName')
            ...
            if(msg.split("/")[1] == "") {
              ...
              const brands = await db('e_brands').select('*').where({bName: msg.split("/")[1]})
              ...
              if(!brands.length) {
                ...
                const products = await db('e_products').select('*').where({companyName: msg.split("/")[1]})
                if(!products.length) {
            ...
            if(msg.split("/")[1] == "") {
              ...
              const brands = await db('e_brands').select('bDetails').where({bName: msg.split("/")[1]})
              if(!brands.length) {
                result = "Your entered brand name is *invalid*.\n\nTry the *brands* endpoint to list the brands that are currently registered"
                sendMsg(from, result, true, "text")
              } 
                ...
                users.forEach(async(user) => {
                  if(user["uPhoneNo"] == from && msg.split("/")[1] === "off") {
                    ...
                    if(!user["nudge"]) {
                      ...
                      await db('e_users').where({uEmail: user["uEmail"], uName: user["uName"]}).update({nudge: false})
                    ...
                    if(user["nudge"]) {
                      ...
                      await db('e_users').where({uEmail: user["uEmail"], uName: user["uName"]}).update({nudge: true}) 
            ...
            const details = await db('e_users').select('*').where({uPhoneNo: from})
...
app.post("/validateEmail", async(req, res) => {
  ...
  try{
    ...
    const emails = await db('e_brands').select('*').where({bEmail: req.body.email})
    if(!emails.length) {
...
app.post("/login", async(req, res) => {
  ...
  try {
    const users = await db('e_brands').select('*').where({bEmail: body.email, pwd: body.pwd})
...
app.post("/addProduct", async(req, res) => {
  ...
  try {
    const pCategory = await db('e_brands').select('bStream').select('bName').where({bEmail: body.bEmail})
    await db('e_products').insert({
      pName      : body.pName,
      bEmail     : body.bEmail,
      pLink      : "",
      pDesc      : body.pDesc,
...
app.post("/fetchProducts", async(req, res) => {
  ...
  try {
    const products = await db('e_products').select('*').where({bEmail: body.email})
    if(!products.length) {
...
app.post("/deleteProduct", async (req, res) => {
  ...
  const del = await db('e_products').where({pName: body.pName, bEmail: body.bEmail, pLoc: body.pLoc}).del()
...
app.post("/validateUser", async(req, res) => {
  ...
  try {
    const email = await db('e_users').select('*').where({uEmail: body.email})
    if(!email.length) {
      const phone = await db('e_users').select('*').where({uPhoneNo: body.phone})
      if(!phone.length) {
...
app.post("/loginUser", async(req, res) => {
  ...
  try {
    const user = await db('e_users').select('*').where({uEmail: body.email, uPwd: body.pwd})
    if(!user.length) {
      res.send("ERROR:Invalid credentials")
    } 
    else {
      ...
      const achievements = await db('e_users').select('achievements').where({uEmail: body.email, uPwd: body.pwd})
      
      if(difference == 1) {
        // increase the login streak
        if(!user[0]["achievements"].includes("l")) await db('e_users').where({uEmail: body.email, uPwd: body.pwd}).update({loginStreak: Number(user[0]["loginStreak"])+1, achievements: user[0]["achievements"]+"l"})
        else await db('e_users').where({uEmail: body.email, uPwd: body.pwd}).update({loginStreak: Number(user[0]["loginStreak"])+1})
        streak++
      } else if(difference > 1) {
        await db('e_users').where({uEmail: body.email, uPwd: body.pwd}).update({loginStreak: 0})
        streak = 0;
      }
      
      console.log("Current date is " + currDate)
      await db('e_users').where({uEmail: body.email, uPwd: body.pwd}).update({lastLogin: currDate})
...
app.post("/recommendations", async(req, res) => {
  ...
  try {
    const users = await db('e_users').select('*').where({uEmail: body.email})
    if(!users.length) {
      ...
      interests.forEach(async (item, ind) => {
        let products = await db('e_products').select('*').where({
        ...
        products = await db('e_products').select('*').where({
...
app.post("/createFeedback", async(req, res) => {
  ...
  if(body.sendTo !== "") {
    sendTo = await db('e_brands').select('*').where({"bName": body.sendTo})
  ...
  const achievements = await db('e_users').select('*').where({uEmail: body.email})
  ...
  if(achievements[0]["achievements"].includes("f")) {
    ...
    await db('e_users').where({uEmail: body.email}).update({achievements: achievements[0]["achievements"]+'f'})
...
app.post("/getBrandProducts", async (req, res) => {
  ...
  const brandProducts = await db('e_products').select('*')
...
app.post("/getLabelProducts", async (req, res) => {
  ...
  for(let label=0; label<labels.length; label++) {
    let products = await db('e_products').select('*').where({label: labels[label]})
...
app.post("/getAchievements", async (req, res) => {
  const achievements = await db('e_users').select('achievements').where({uEmail: req.body.email})
...
app.post("/updateVerify", async (req, res) => {
  ...
  if(achievements[0]["achievements"].includes("v")) {
    ...
    await db('e_users').where({uEmail: req.body.email}).update({achievements: achievements[0]["achievements"]+'v'})
    res.send("Achievement")
  }
})
...
app.get("/getChartValues", async(req, res) => {
  const males = await db("e_users").count('*').where({uGender: "m"})
    const females = await db("e_users").count('*').where({uGender: "f"})
    
    const kids = await db("e_users").count('*').where({uAgeGroup: "kids"})
    const youth = await db("e_users").count('*').where({uAgeGroup: "youth"})
    const adult = await db("e_users").count('*').where({uAgeGroup: "adult"})
    const old = await db("e_users").count('*').where({uAgeGroup: "old"})
    
    const clothing = await db("e_products").count('*').where({pCategory: "clothing"})
    const food = await db("e_products").count('*').where({pCategory: "food"})
    const furniture = await db("e_products").count('*').where({pCategory: "furniture"})
    const electronics = await db("e_products").count('*').where({pCategory: "electronics"})
...
app.post("/statEmails", async (req, res) => {
  ...
  const users = await db('e_users').select('*').where({uAgeGroup: body.ageGroup, uGender: body.gender})
...
app.post("/processPayments", async (req, res) => {
  ...
  try {
    ...
    await db('e_users').where({uEmail:email}).update({orderID: JSON.parse(payment.body).payment.id})
...
app.post("/addAmountTotag", async(req, res) => {
  ...
  await db('e_users').where({uEmail:body.email}).update({tagAmount: body.amount})
  res.send("OKAY")
})
...
app.get("/checkOrder", async(req, res)=> {
  ...
  const order = await db('e_users').select('*').where({tag: tag})
  if(!order.length) {
...
app.get("/processPayment", async(req, res) => {
  ...
  if(!order.length) {
    ...
    if(order[0]['tagAmount'] == 0) {
      ...
      await db('e_users').where({tag: tag}).update({orderID: "", tagAmount: 0})
...
app.get("/checkUser", async(req, res)=> {
    ...
    if(token == undefined || token.split(' ')[1] !== "vishalvats") {
     res.status(200).send("No Authorization header") 
    }
